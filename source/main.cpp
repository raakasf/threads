...
int main()
{
  thread reaper_thread(thread_learning::reaper);
  for (int i = 0; i < 10; i++)
  {
    std::lock_guard<std::mutex> lk(thread_learning::cv_w);
    thread_learning::watches.push_back(thread(thread_learning::watch, i));
  }
